<template>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <template v-if="isLoading">
        <div v-for="i in 3" :key="i" class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-md p-6 animate-pulse">
          <div class="flex items-center justify-between mb-4">
            <div class="h-8 w-24 bg-gray-200 rounded-md"></div>
            <div class="h-10 w-10 bg-gray-200 rounded-full"></div>
          </div>
          <div class="h-12 w-32 bg-gray-200 rounded-md mb-4"></div>
      ...
      <template v-else>
        <div
          ...
          :key="card.id"
          class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-md p-6 transition-all duration-300 hover:shadow-lg hover:-translate-y-1
  flex flex-col"
        >
          <!-- Card Header -->
          <div class="flex items-center justify-between mb-4">
            <h3 class="font-semibold text-gray-600">{{ card.title }}</h3>
          ...
          <!-- Main Value -->
          <div class="mb-4">
            <span class="text-4xl font-bold text-gray-800">{{ card.value }}</span>
            <span class="text-xl font-medium text-gray-500 ml-1">{{ card.unit }}</span>
          </div>
